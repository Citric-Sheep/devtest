...
class GenerateDataset():
    ...
    def load_elevator_variables(self):
        try:
            ...
            # Validate structure
            if not all(key in data for key in ['floor_capacities', 'floor_types', 'rows_generated', 
                                               'floor_number', 'min_time_interval_seconds', 
                                               'max_time_interval_seconds', 'peak_hours',
                                               'peak_multiplier', 'random_minutes_range']):
            ...
            if not isinstance(data['rows_generated'], int) or data['rows_generated'] <= 0:
            ...
            for interval_key in ['min_time_interval_seconds', 'max_time_interval_seconds']:
            ...
            if not isinstance(data['interval_per_floor_seconds'], (int, float)) or not 0 <= data['interval_per_floor_seconds']:
                raise ValueError("Invalid 'interval_per_floor_seconds' value.")

            # Validate peak_hours
            if not isinstance(data['peak_hours'], list) or not all(isinstance(interval, dict) for interval in data['peak_hours']):
                raise ValueError("Invalid 'peak_hours' format.")
            for peak_interval in data['peak_hours']:
                if not all(key in peak_interval for key in ['start', 'end']) or not isinstance(peak_interval['start'], int) or not isinstance(peak_interval['end'], int):
                    raise ValueError("Invalid peak hour interval.")

            # Validate peak_multiplier
            if not isinstance(data['peak_multiplier'], (int, float)) or not 0 <= data['peak_multiplier'] <= 1:
                raise ValueError("Invalid 'peak_multiplier' value.")

            # Validate random_minutes_range
            if not isinstance(data['random_minutes_range'], dict) or not all(key in data['random_minutes_range'] for key in ['min', 'max']) or not isinstance(data['random_minutes_range']['min'], int) or not isinstance(data['random_minutes_range']['max'], int):
    ...
    def floor_weights_mapping(self):
        ...
        floor_capacities_mapping = {
            floor: self.floor_capacities.get(self.floor_types.get(str(floor), 'Residential'))
        ...
        floor_weights_mapping = {floor: base_weight + (capacity - min_capacity) / (max_capacity - min_capacity) for floor, capacity in floor_capacities_mapping.items()}

        # Normalize weights so that the sum is 1
        total_weight = sum(floor_weights_mapping.values())
        floor_weights_mapping = {floor: weight / total_weight for floor, weight in floor_weights_mapping.items()}
        self.weight_list = list(floor_weights_mapping.values())
    def pick_random_floor_weighted(self):
        random_floor = random.choices(range(1, self.floor_number + 1), weights=self.weight_list)[0]
        return random_floor
    def pick_next_door(self):
        # Ensure demand_floor is different from next_floor
        while True:
            next_floor = random.choices(range(1, self.floor_number + 1), weights=self.weight_list)[0]
    ...
    def calculate_interval_minutes(self):
        ...
        current_floor_interval_lag = (
            self.min_time_interval_seconds if current_floor_distance <= 1 else min(self.max_time_interval_seconds, current_floor_distance * self.interval_per_floor_seconds)
        ...
        next_floor_interval_lag = (
            self.min_time_interval_seconds if next_floor_distance <= 1 else min(self.max_time_interval_seconds, next_floor_distance * self.interval_per_floor_seconds)
        ) / 60  # Convert seconds to minutes

        # Assuming self.start_time is a datetime object
        if any(interval['start'] <= self.start_time.hour < interval['end'] for interval in self.peak_hours):
            peak_interval_multiplier = self.peak_multiplier
        else:
            peak_interval_multiplier = 1.0  # No multiplier outside of peak hours
        random_minutes = random.randint(self.random_minutes_range["min"], self.random_minutes_range["max"])
