...
class ElevatorViewSetTestCase(TestCase):
    ...
    def test_call_elevator_same_floor(self):
        elevator = Elevator.objects.create(number_of_floors= 10, elevator_max_speed=2.0, current_floor=0)
    ...
    def test_call_elevator_different_floor(self):
        ...
        person = Person.objects.create(name='Jon Doe', age=44)
        call1 = ElevatorCall.objects.create(elevator=elevator, origin_floor=2, target_floor=6,person = person)
        response = self.client.post('/elevator_api/elevator/call_elevator/',data={'call_id':call1.id, 'elevator_id':elevator.id, 'person_id':person.id}, format='json')
        movement_data = response.data.get('movement', {})
        self.assertEqual(movement_data.get('total_traveled_floors'), 10)
        elevator.refresh_from_db()
        self.assertEqual(elevator.current_floor, 6)
