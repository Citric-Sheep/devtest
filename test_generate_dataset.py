...
def test_floor_weights_mapping(generator):
    ...
    generator.floor_capacities = {  "Residential": 20,
    "Social": 100,
    "entrance": 200,
    "Company": 40
    }
    
    generator.floor_types = {   "1": "entrance",
    "10": "Social",
    "2": "Company"}
    
    generator.floor_number = 10
    
    expected_result = [0.43043478260869567, 0.08260869565217391, 0.03913043478260869, 0.03913043478260869, 0.03913043478260869, 0.03913043478260869, 0.03913043478260869, 0.03913043478260869, 0.03913043478260869, 0.21304347826086953]
    # Call the method to be tested
    generator.floor_weights_mapping()

    # Check the results/assertions
    assert hasattr(generator, 'weight_list')  # Check if weight_list attribute is created
    assert len(generator.weight_list) == generator.floor_number  # Check if the length is correct
...
def test_pick_random_floor_weighted(generator):
    ...
    assert 1 <= result <= generator.floor_number  # Check if the result is within the valid floor range
...
def test_pick_next_door(generator):
    ...
    # Check the results/assertions
    assert isinstance(result, int)  # Check if the result is an integer
    assert result != generator.demand_floor  # Check if the result is different from the demand floor
    assert 1 <= result <= generator.floor_number  # Check if the result is within the valid floor range
...
def test_calculate_interval_minutes(generator):
    ...
    generator.start_time = datetime(2023, 1, 1, 12, 0, 0)  # Set a specific start time for testing
    ...
    # Check the results/assertions
    assert isinstance(result, (int, float))  # Check if the result is a valid numeric type
