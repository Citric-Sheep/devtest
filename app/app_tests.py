...
class MLDataIntegrationTestCase(unittest.TestCase):
    ...
    def test_export_ml_features_csv(self):
        """Test exporting ML features as CSV"""
        response = self.app.get(f'/api/ml-data/features/{self.building.id}?format=csv')
        self.assertEqual(response.status_code, 200)
        self.assertIn('text/csv', response.content_type)
        self.assertIn('Content-Disposition', response.headers)
        self.assertIn('target_floor', response.get_data(as_text=True))
    
    def test_export_ml_features_json(self):
        """Test exporting ML features as JSON"""
        response = self.app.get(f'/api/ml-data/features/{self.building.id}?format=json')
        self.assertEqual(response.status_code, 200)
        ...
        if data['total_samples'] > 0:
            self.assertIn('target_floor', data['features'][0])
    
    def test_export_ml_features_missing_building_id(self):
        """Test export with missing building_id"""
        response = self.app.get('/api/ml-data/features/')
        self.assertEqual(response.status_code, 404)  # Changed from 400 to 404 as the route requires building_id
    
    def test_export_ml_features_nonexistent_building(self):
        """Test export with non-existent building_id"""
        response = self.app.get('/api/ml-data/features/9999')
