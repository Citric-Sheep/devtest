...
def get_ml_features(building_id):
    """Get ML features for a building, optionally export as CSV"""
    ...
    for item in features:
        flat_item = {
            ...
            **item['time_features'],
            **{f"elevator_{k}": v for k, v in item['elevator_features'].items()},
            **{f"ctx_{k}": v for k, v in item['contextual_features'].items()}
        }
        flat_features.append(flat_item)
    ...
    if export_format == 'csv':
        ...
        return output.getvalue(), 200, {
            ...
            'Content-Disposition': f'attachment; filename=elevator_features_building_{building_id}.csv'
        }
    else:  
        return jsonify({
            'building_id': building_id,
